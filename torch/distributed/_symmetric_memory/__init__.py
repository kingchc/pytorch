...
def _mxn_cast_mesh_to_ints(
    ...
    """Extract ``(mesh_dims, start_rank)`` from a ``DeviceMesh`` in the
    form the underlying ``ncclReshardUserWindow`` C API expects.
...
def _mxn_cast_placements_to_ints(
    ...
    encoding consumed by ``ncclReshardMesh_t``: ``Shard(d) -> d``,
...
def mxn_cast(
    ...
    meshes, backed by the ``ncclReshardUserWindow`` primitive.
    ...
    underlying ``ncclReshardMesh_t`` constraints).
    ...
    # passes data_ptr=NULL on that side to ncclReshardUserWindow (the
