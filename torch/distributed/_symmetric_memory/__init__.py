...
def _mxn_cast_mesh_to_ints(
    ...
    if mesh_ndim == 1:
        ...
        raise ValueError(
            f"mxn_cast: DeviceMesh must be 1-D or 2-D, got {mesh_ndim}-D"
        )
    ...
    try:
        ...
        raise ValueError(
            f"mxn_cast: DeviceMesh rank {exc.args[0]} is not in group {group!r}"
        ) from None
    ...
    if group_ranks != expected:
        raise ValueError(
            "mxn_cast: DeviceMesh ranks must map to a contiguous row-major "
...
def _mxn_cast_placements_to_ints(
    ...
    encoding consumed by ``ncclXferDistTensor_t::placements[]``:
    ``Shard(d) -> d``, ``Replicate() -> -1``.  Lists shorter than
    ``mesh_ndim`` are padded with ``-1``.  Partial placements are not
    supported.
    """
    from torch.distributed.tensor.placement_types import Replicate, Shard

    if mesh_ndim not in (1, 2):
        raise ValueError(f"mxn_cast: mesh_ndim must be 1 or 2, got {mesh_ndim}")
    if len(placements) > mesh_ndim:
        raise ValueError(
            f"mxn_cast: got {len(placements)} placements for a {mesh_ndim}-D "
            "DeviceMesh"
        )
    ...
    for i, p in enumerate(placements):
        if isinstance(p, Shard):
            if shard_mesh_dim is not None:
                raise ValueError(
                    "mxn_cast: only one mesh dimension may use Shard placement"
                )
            shard_dim = int(p.dim)
            if shard_dim < 0:
            ...
            if shard_dim < 0 or shard_dim >= tensor_ndim:
                raise ValueError(
                    f"mxn_cast: Shard({p.dim}) is out of range for a "
...
def mxn_cast(
    ...
    M-to-N cast (resharding) of a 1-D, 2-D, or 3-D tensor between two rank
    meshes, backed by the ``ncclXferReshardWithWindow`` primitive.
    ...
    underlying ``ncclXferDistTensor_t::placements[]`` constraints).
    ...
    src_mesh_dims, src_start_rank = _mxn_cast_mesh_to_ints(src_mesh, group)
    dst_mesh_dims, dst_start_rank = _mxn_cast_mesh_to_ints(dst_mesh, group)
    src_placement_ints = _mxn_cast_placements_to_ints(
        src_placements, src_mesh.mesh.dim(), len(src_local_shape)
    )
    dst_placement_ints = _mxn_cast_placements_to_ints(
    ...
    # passes dataPtr=NULL on that side to ncclXferReshardWithWindow (the
...
def mxn_cast_finalize() -> None:
    """Release the reshard library's internal caches and transpose buffer.
    ...
    you want a clean release::
        ...
        symm_mem.mxn_cast_finalize()
        dist.destroy_process_group()
    """
    try:
        torch.ops.symm_mem.nccl_mxn_cast_finalize()
    except Exception:
        pass
